...
static Type *declspec(Token **rest, Token *tok, VarAttr *attr) {
  ...
  for (;;) {
    ...
    case TK_restrict:
    case TK_Noreturn:
    case TK_volatile: continue;
    case TK_extern: attr->is_extern = true; continue;
    case TK_inline: attr->is_inline = true; continue;
    case TK_static: attr->is_static = true; continue;
    case TK_typedef: attr->is_typedef = true; continue;
    case TK_thread_local: attr->is_tls = true; continue;
    }
...
static Type *type_suffix(Token **rest, Token *tok, Type *ty) {
  ...
  if (consume(&tok, tok, "[")) {
    while (tok->kind == TK_static || tok->kind == TK_const || tok->kind == TK_volatile ||
...
static void case_range(Token **rest, Token *tok, Node *node) {
  ...
  for (Node *n = current_switch->case_next; n; n = n->case_next)
    if ((less_eq(ty, n->lo, lo) && less_eq(ty, lo, n->hi)) ||
      (less_eq(ty, lo, n->lo) && less_eq(ty, n->lo, hi)))
...
static void label_stmt(Token **rest, Token *tok, Node **stmt) {
  for (;;) {
...
static int64_t eval2(Node *node, char ***label) {
  ...
  switch (node->kind) {
  case ND_ADD:
    return eval_sign_extend(ty, eval2(lhs, label) + eval2(rhs, label));
  case ND_SUB:
    return eval_sign_extend(ty, eval2(lhs, label) - eval(rhs));
  case ND_MUL:
  ...
  if (label) {
    ...
    case ND_ADDR:
    case ND_DEREF:
      return eval2(lhs, label);
    case ND_MEMBER:
      return eval2(lhs, label) + node->member->offset;
    case ND_LABEL_VAL:
